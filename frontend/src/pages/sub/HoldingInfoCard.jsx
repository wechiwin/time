    ...
    return (
        <>
            <div className="card p-5 bg-white rounded-xl shadow-sm border border-gray-100">
                {/* 头部：名称、代码、按钮、总收益 */}
                <div
                    className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 mb-4 border-gray-100">
                    <div>
                        <h1 className="text-xl font-bold text-gray-900">{fundInfo?.ho_name || code}</h1>
                        <div className="text-sm text-gray-500 mt-1 flex items-center gap-3">
                            <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-600 font-mono">{code}</span>
                            <span>{fundInfo?.ho_type}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-6">
                            ...
                            {hasStats ? (
                                <>
                                    {globalStats.isHolding ? (
                                        <span className="text-indigo-600">{t('info_status_holding', '持仓中')}</span>
                                    ) : (
                                        <span className="text-gray-400">{t('info_status_cleared', '已清仓')}</span>
                                    )}
                        ...
                        <div className="font-semibold text-gray-900">
                            {hasStats ? (
                                <>
                                    ...
                                    {globalStats.isHolding}
                                    <span className="text-xs ml-0.5">{t('info_hold_day_unit', '天')}</span>
                                </>
                            ) : '-'}
                        </div>
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <div
                            className={`font-semibold ${hasStats ? (globalStats.totalProfit >= 0 ? 'text-red-600' : 'text-green-600') : 'text-gray-900'}`}>
                            {hasStats ? (
                                <>
                    ...
                    <div className="space-y-1">
                        ...
                        <div
                            className={`font-semibold ${hasStats ? (globalStats.cumulativeReturnRate >= 0 ? 'text-red-600' : 'text-green-600') : 'text-gray-900'}`}>
                            {hasStats ? (
                                <>
