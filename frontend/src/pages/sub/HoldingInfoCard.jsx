...
export default function HoldingInfoCard({
    ...
    return (
        <>
            <div className="card p-5 bg-white dark:bg-gray-700 rounded-xl shadow-sm border border-gray-100 dark:border-gray-600">
                {/* 头部：名称、代码、按钮、总收益 */}
                <div
                    ...
                    <div>
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-3">
                            <span className="bg-gray-100 dark:bg-gray-600 px-2 py-0.5 rounded text-gray-600 dark:text-gray-300 font-mono">{code}</span>
                            ...
                            {hasStats ? (
                                <>
                                    {globalStats.isHolding ? (
                                        <span className="text-indigo-600 dark:text-indigo-400">{t('info_status_holding', '持仓中')}</span>
                                    ) : (
                                        <span className="text-gray-400 dark:text-gray-500">{t('info_status_cleared', '已清仓')}</span>
                                    )}
                        ...
                        <div className="font-semibold text-gray-900 dark:text-gray-200">
                            {hasStats ? (
                                <>
                                    ...
                                    <span className="text-xs ml-0.5 text-gray-600 dark:text-gray-400">{t('info_hold_day_unit', '天')}</span>
                                </>
                            ) : '-'}
                        </div>
                    </div>
